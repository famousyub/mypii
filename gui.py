...
class Tftp_gui(object):
    ...
    def write_command(self):
        ...
        if not os.path.isfile(file_name):
            ...
            if c.write(file_name, os.path.basename(file_name) if len(alt_filename) == 0 else alt_filename):
                print('Success')

    def read_command(self):
        c,file_name, alt_filename = TFTPClient(self._host.get(), self._port.get()), self.remote_file.get(), self.alt_filename.get()
        if c.read(file_name, file_name if len(alt_filename) == 0 else alt_filename):
